    ...
    <div dir="rtl" className="bg-white pb-24 text-slate-800 lg:pb-0">
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:py-16 lg:px-8">
          <div className="space-y-5">
            ...
            <h1 className="text-3xl font-extrabold leading-tight tracking-tight text-[#0b2545] sm:text-4xl lg:text-[2.75rem] lg:leading-tight">
              استعد لاجتياز اختبار <span className="text-[#B8860B]">CPHQ الأمريكي</span> واصنع مستقبلك في الجودة الصحية
            </h1>
            ...
            <div className="flex flex-wrap gap-3">
              <LeadFormModal
                ...
                triggerClassName={cn(BTN, ORANGE)}
                triggerLabel={<>احجز الآن <ChevronLeft className="size-4" /></>}
              />
              <a href="#apply" className={cn(BTN, "border border-[#0b2545]/20 bg-white text-[#0b2545] shadow-sm hover:bg-[#0b2545]/5")}>تعرّف على التفاصيل</a>
            </div>
      ...
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
          <div className="mx-auto mb-8 max-w-2xl text-center">
            <h2 className="text-3xl font-extrabold tracking-tight text-[#0b2545]">ماذا ستحصل بعد اجتياز CPHQ؟</h2>
            <p className="mt-3 text-slate-600">الكورس وسيلة — والنتيجة هي ما يهمّك. إليك ما تفتحه لك شهادة CPHQ فعليًا.</p>
          </div>
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {OUTCOMES.map((o) => (
              <div key={o.title} className="rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-sm transition hover:shadow-md">
                <span className="mx-auto grid size-12 place-items-center rounded-xl bg-[#B8860B]/10 text-[#B8860B]"><o.icon className="size-6" /></span>
                <h3 className="mt-4 font-bold text-[#0b2545]">{o.title}</h3>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map((f) => (
            <div key={f.title} className="rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-sm transition hover:shadow-md sm:text-right">
              <div className="mx-auto grid size-16 place-items-center rounded-2xl bg-[#B8860B]/10 text-[#B8860B] sm:mx-0 sm:size-16"><f.icon className="size-8" /></div>
              <h3 className="mt-4 text-lg font-bold text-[#0b2545]">{f.title}</h3>
      ...
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
          <h2 className="text-center text-3xl font-extrabold tracking-tight text-[#0b2545]">هل الكورس مناسب لك؟</h2>
          <p className="mx-auto mt-3 max-w-xl text-center text-slate-600">إذا كنت من أيٍّ من هؤلاء، فالبرنامج مصمَّم لك.</p>
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {AUDIENCE.map((a) => (
              <div key={a} className="inline-flex items-center gap-2.5 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-[#0b2545]">
                <CheckCircle2 className="size-5 shrink-0 text-emerald-600" /> {a}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto mb-8 max-w-2xl text-center">
          <div className="flex flex-wrap items-center justify-center gap-2.5">
            <BrandImage
              ...
              fallback={
                <span className="grid size-8 place-items-center rounded-lg bg-[#0b2545] text-white"><Building2 className="size-4" /></span>
              }
            />
            <h2 className="text-3xl font-extrabold tracking-tight text-[#0b2545]">لماذا IMETS Medical School؟</h2>
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr style={{ backgroundColor: NAVY }} className="text-white">
                ...
                <th className="w-[26%] bg-emerald-500/15 p-4 text-center text-lg font-extrabold text-[#e8c14d]">IMETS</th>
            ...
            <tbody>
              {COMPARE.map((r, i) => (
                <tr key={r.feature} className={cn("border-t border-slate-100", i % 2 === 1 && "bg-slate-50/50")}>
                  <td className="px-4 py-5 text-right text-[15px] font-semibold text-[#0b2545]">{r.feature}</td>
        ...
        <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="flex flex-col justify-center rounded-2xl bg-[#0b2545] p-6 text-white shadow-xl ring-1 ring-black/5">
              ...
              <LeadFormModal
                path={trackPath} courseName={COURSE} webhookUrl={WEBHOOK}
                triggerClassName={cn(BTN, ORANGE, "mt-5 w-fit")}
      ...
      <section dir="rtl" className="border-y border-slate-200 bg-slate-50">
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:px-8">
          <div className="space-y-5">
            ...
            />
            <h2 className="text-3xl font-extrabold tracking-tight text-[#0b2545]">عن IMETS Medical School</h2>
            <p className="text-xl font-bold leading-relaxed text-[#0b2545]">
            ...
            <a href="#apply" className={cn(BTN, ORANGE)}>انضم إلى الدفعة القادمة <ChevronLeft className="size-4" /></a>
          </div>
      ...
      <section dir="rtl" className="bg-white">
        <div className="mx-auto max-w-5xl px-4 pt-14 sm:px-6 lg:px-8">
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-center shadow-sm">
            <p className="font-bold text-[#0b2545]">هل لديك أي سؤال قبل التسجيل؟</p>
            <p className="mt-1 text-sm text-slate-500">تواصل معنا مباشرة وسنجيبك فورًا.</p>
            <div className="mt-4 flex flex-wrap items-center justify-center gap-3">
              <a href="https://wa.me/201115782721" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[#25D366] px-5 py-2.5 text-sm font-bold text-white shadow-sm transition hover:brightness-105">
                <MessageCircle className="size-4" /> واتساب
              </a>
              <a href="tel:+201115782721" className="inline-flex items-center gap-2 rounded-full bg-[#0b2545] px-5 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-[#13315c]">
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 flex items-center gap-3 border-t border-slate-200 bg-white/95 p-3 backdrop-blur lg:hidden">
        <LeadFormModal
          path={trackPath} courseName={COURSE} webhookUrl={WEBHOOK}
          triggerClassName={cn(BTN, ORANGE, "w-full flex-1")}
          triggerLabel={<>احجز الآن <ChevronLeft className="size-4" /></>}
        />
      </div>
