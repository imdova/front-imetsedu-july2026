...
import { MessageSquareText, ClipboardList, Coins, Wallet, FileText, Link2, Upload } from "lucide-react";
...
/** Office tabs. `initialTab` (from `?tab=`) opens a specific one. */
export function OfficePanel({ initialTab }: { initialTab?: string }) {
  const defaultTab = TABS.some((t) => t.value === initialTab) ? initialTab! : "messages";
