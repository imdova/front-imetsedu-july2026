...
/** Google / Facebook OAuth entry points (demo: backend redirect not wired). */
export function SocialButtons() {
  const t = useTranslations("Auth");
  const notify = (provider: string) =>
    toast.info(`${provider} OAuth is wired to the backend in production.`);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Button variant="outline" type="button" onClick={() => notify("Google")}>
          <GoogleIcon />
          {t("google")}
        </Button>
        <Button variant="outline" type="button" onClick={() => notify("Facebook")}>
          <FacebookIcon />
          {t("facebook")}
        </Button>
      </div>
