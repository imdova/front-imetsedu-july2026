...
export function CourseCurriculum({
  ...
  return (
    <div dir={ar ? "rtl" : "ltr"} className="w-full">
      <ol className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-5 lg:gap-6">
        {modules.map((m, i) => {
          ...
          const cardSurface = expanded
            ...
            : i % 2 === 0
              ...
              : "border-border/50 bg-white hover:border-primary/20 hover:shadow-sm dark:bg-primary/[0.06]";
          ...
          return (
            ...
            >
              <button
                ...
                className="flex h-full w-full items-start gap-3 px-4 py-4 text-start sm:gap-3.5 sm:px-5 sm:py-5"
              >
                <span
                  className={cn(
                    "grid size-9 shrink-0 place-items-center rounded-full font-heading text-sm font-bold tabular-nums transition-all",
                    expanded
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-heading text-base font-bold tracking-tight text-foreground sm:text-[1.05rem]">
                    {title}
                  </p>

                  {topics.length > 0 ? (
                    <p className="mt-1.5 text-xs font-medium tracking-wide text-primary/80">
                      {topics.join(" · ")}
                    </p>
                  ) : null}
                  ...
                  <span
                    aria-hidden
                    className={cn(
                      "mt-3 block h-0.5 max-w-[10rem] rounded-full",
                      expanded
                        ? "bg-gradient-to-r from-primary via-primary to-amber-400"
                        : "bg-primary/15",
                    )}
                    style={{
                      width: `${Math.min(100, 32 + ((i + 1) / Math.max(modules.length, 1)) * 68)}%`,
                    }}
                  />
              ...
              {expanded && (
                <div className="space-y-4 border-t border-primary/10 bg-gradient-to-b from-primary/[0.03] to-transparent px-4 py-4 sm:px-5 sm:py-5 md:grid md:grid-cols-2 md:gap-8 md:space-y-0">
                  <div className="space-y-4">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <DeliveryChip
                        icon={Clock}
                        label={t("liveSessionsCount", { count: sessionCount || 1 })}
                    ...
                    {sessionCount > 0 ? (
                      <>
                        ...
                        <ul className="mt-2">
                          {m.lessons.map((l, j) => {
                            const lt =
                              (ar ? l.titleAr || l.titleEn : l.titleEn || l.titleAr) ||
                              `Session ${j + 1}`;
                            return (
                              <li
                                key={`${lt}-${j}`}
                              ...
                              >
                                ...
                                <span className="inline-flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                                  {l.duration && (
                                    <span className="tabular-nums">{l.duration}</span>
                                  )}
